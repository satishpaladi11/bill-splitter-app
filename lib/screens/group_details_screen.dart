...
class GroupDetailsScreen extends StatelessWidget {
  ...
  Map<String, double> calculateBalances(
    ...
    for (final e in expenses) {
      ...
      // check for custom splits saved on the expense (percentages 0-100)
      Map<String, double>? splits;
      if (e['splits'] is Map) {
        try {
          splits = Map<String, double>.fromEntries(
            (e['splits'] as Map).entries.map((ent) => MapEntry(ent.key.toString(), (ent.value as num).toDouble())),
          );
        } catch (_) {
          splits = null;
        }
      ...
      for (final m in members) {
        if (!members.contains(payer)) continue;
        // compute share for this member
        double share;
        if (splits != null && splits.containsKey(m)) {
          share = amount * (splits[m]! / 100.0);
        } else {
          share = members.isNotEmpty ? amount / members.length : 0.0;
        }
        balance[m] = (balance[m] ?? 0.0) + ((m == payer) ? amount - share : -share);
      }
    }
    return balance;
  }
            ...
            IconButton(
              ...
              onPressed: () async {
                ...
                final members =
                    ...
                    <Map<String, dynamic>>[];
                final expensesRaw =
                    (group['expenses'] as List<dynamic>?) ?? <dynamic>[];
                              ...
                              try {
                                ...
                                if (byteData != null) {
                                  ...
                                  final file = await File(
                                    '${tempDir.path}/group_summary.png',
                                  ).writeAsBytes(pngBytes);
                                  await Share.shareXFiles([
                                    XFile(file.path),
                                  ], text: 'Group summary from Simplify Split');
                                } else {
                                  ScaffoldMessenger.of(ctx).showSnackBar(
                                    const SnackBar(
                ...
                builder: (context, Box groups, _) {
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      ...members.asMap().entries.map((entry) {
                                        ...
                                        final isDefault = m['isDefaultUser'] == true;
                                        return Chip(
                                          ...
                                          onDeleted: isDefault
                                              ...
                                              : () async {
                                                  ...
                                                  final updatedMembers = (g['members'] as List<dynamic>?)
                                                          ?.map((mm) => mm is Map ? Map<String, dynamic>.from(mm) : {'name': mm.toString()})
                                                          .toList() ??
                                                      <Map<String, dynamic>>[];
                                                  if (idx >= 0 && idx < updatedMembers.length) {
                                                    updatedMembers.removeAt(idx);
                                                    await box.put(groupId, {...g, 'members': updatedMembers});
                                                  }
                                                },
                                        );
                                      }).toList(),
                                              ...
                                              actions: [
                                                ...
                                                ElevatedButton(
                                                  onPressed: () {
                                                    ...
                                                    // Share invite
                                                    Share.share(
                                                      "Join my group '${g['name'] ?? ''}' on Simplify Split. Group Code: $groupId",
                                                    );
                                                    Navigator.pop(ctx);
                                                  },
                                                  child: const Text(
                  ...
                  final descOrNote = desc ?? 'No note';
                                  ...
                                  if (ts != null) {
                                    ...
                                    final y = local.year.toString().padLeft(
                                      4,
                                      '0',
                                    );
                                    final mo = local.month.toString().padLeft(
                                      2,
                                      '0',
                                    );
                                    final d = local.day.toString().padLeft(
                                      2,
                                      '0',
                                    );
                                    final h = local.hour.toString().padLeft(
                                      2,
                                      '0',
                                    );
                                    final min = local.minute.toString().padLeft(
                                      2,
                                      '0',
                                    );
                                  ...
                                  final leadText = (payer.isNotEmpty)
                                      ? payer[0].toUpperCase()
                                      : 'E';
                                  ...
                                  if (desc != null) {
                                    ...
                                    // show subtle placeholder instead of empty space
                                            ...
                                            onPressed: () {
                                              ...
                                              final originalIndex = exList
                                                  .indexWhere((item) {
                                                    ...
                                                    if (item['amount'] == e['amount'] && (item['desc'] ?? '') == (e['desc'] ?? '') && (item['payer'] ?? '') == (e['payer'] ?? '')) {
                                                      return true;
                                                    }
                                                    return false;
                                                  });
